...
namespace TAMU.GeoInnovation.Applications.Census.ReferenceDataImporter.PostgreSQL.FileLayouts.Implementations.Tiger2010.StateFiles
{
    public class CountySub2010File : AbstractTiger2010ShapefileStateFileLayout
    {
            ...
            : base(stateName)
        {
            ...
            //Parameter not supported by postgres
            //SQLCreateTable += "CONSTRAINT PK_" + OutputTableName + "_cousubFp PRIMARY KEY (cousubFp10) WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = ON, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]";
            SQLCreateTable += "CONSTRAINT PK_" + OutputTableName + "_cousubFp PRIMARY KEY (cousubFp10)";
            SQLCreateTable += ");";
